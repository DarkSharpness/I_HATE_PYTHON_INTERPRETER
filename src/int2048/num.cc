...
class int2048 : public array <uint64_t>  {
  public:
    ...
    static constexpr uint64_t unit[20] = {
        1,
        10,
        100,
        1000,
    ...
    static inline uint64_t Round(double num) {
        return num + 0.5;
    }
    ...
    friend inline bool operator <(const int2048 &X,const int2048 &Y) {
        ...
        else return Abs_Compare(Y,X) == judgeMap[X.sign]; 
    }
    /* Compare X and Y*/
    friend inline bool operator >(const int2048 &X,const int2048 &Y) {
        if(X.sign != Y.sign) return X.sign < Y.sign;
        else return Abs_Compare(X,Y) == judgeMap[X.sign];
    }
    /* Compare X and Y*/
    friend inline bool operator <=(const int2048 &X,const int2048 &Y) {
        if(X.sign != Y.sign) return X.sign > Y.sign;
    ...
    /* Compare X and Y*/
    friend inline bool operator !(const int2048 &X) {
        return !X.back();
    }
    ...
    /* X * (-1) */
    friend inline int2048 operator -(int2048 &&X) {
        return X.reverse();
    }
...
int2048 dto2048(double tmp) {
    ...
    size_t idx = str.find('.');
    if(idx != std::string::npos) str.resize(idx);
    int2048 ans;
    ans.read(str);
    return ans;
}
